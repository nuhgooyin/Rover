...
int val, xValue, yValue, defX, defY;
...
defX = analogRead(joyX);
defY = analogRead(joyY);
}
void advance() // move forward
{
digitalWrite(pinLB,HIGH); // right wheel moves forward
...
{
digitalWrite(pinLB,LOW); // left wheel moves backward
digitalWrite(pinRB,LOW); // right wheel moves backward
analogWrite(pinLF,255);
analogWrite(pinRF,255);
}
